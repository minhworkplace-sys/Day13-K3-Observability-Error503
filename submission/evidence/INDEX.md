...
|---|---|---|---|
| 1 | Kết quả `validate_logs.py` cuối cùng | `validate-logs.txt` | ✅ 100/100 — 164 record, 81 correlation ID, 0 PII leak |
...
| — | Prompt v1/v2 và bằng chứng rollback | `traces-prompt-versions.txt`, `prompt-labels-after-rollback.txt` | ⚠️ đủ trace ID và trạng thái label; còn thiếu ảnh chụp màn hình Langfuse |
| — | Evidence checkpoint 0 và 1 | `health.png`, `cp1.png` | ✅ |

## Luồng đọc theo Metrics → Traces → Logs

1. **Metrics** — `metrics-comparison.json`: p95 nhảy từ 155 ms lên 2.654 ms (+2.499 ms) trong khi p50 đứng yên 152 ms và `error_breakdown` rỗng. Chậm ở phần đuôi, không phải lỗi.
2. **Traces** — `trace-waterfall.txt`: trong 2,656s của trace, span `rag_retrieve` chiếm 2,502s (94,2%), `llm_generate` chỉ 0,151s. Trace đối chứng lúc incident tắt có `rag_retrieve` = 0,000s.
3. **Logs** — `challenge-timeline.txt`: 5 request gửi song song nhưng bị xử lý tuần tự, cách nhau đúng 2,65s, request cuối chờ 13,32s trong khi log chỉ ghi `latency_ms=2656`.
...
Toàn bộ trace ID trong evidence thuộc project Langfuse `cmsoa3pmb00asad0ctyui0lc6`. Mở bằng cặp key khác sẽ trả về 404 chứ không phải trace không tồn tại.
